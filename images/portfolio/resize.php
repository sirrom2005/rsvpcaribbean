...
/***********************************************************************************************
	...
	Property: SmiledSoft.com
	URL: http://smiledsoft.com/demos/
	Date: 27.08.2004
***********************************************************************************************/
...
class hft_image
{
		...
		function hft_image($file_original){
			...
			$file_original = dirname(__FILE__) . '/' . $file_original;
		...
		function set_parameters($jpeg_quality="100", $use_gd2=true) {
        ...
        function _output_image($destination_file, $image_type, $image){
			...
			if($image){
                switch($image_type) {
                    case 'JPEG':
                    case 'JPG':
						//if(!$destination_file) header("Content-type: image/jpeg");
                        if(!$destination_file): 
                    ...
                    break; 
                    case 'PNG': 
						if(!$destination_file) header("Content-type: image/png");
...
$img = isset($_GET['img'])? base64_decode($_GET['img']) : null;
$file = '';
$size = isset($_GET['size'])? $_GET['size'] : 100;
$sizeWidth = false;

if(isset($img) && file_exists(dirname(__FILE__) . '/' . $img))
{
	$imageData = getimagesize(dirname(__FILE__) . '/' . $img);
	if($imageData[0] > $imageData[1])
	{
		$sizeWidth = true;
		$size = ($size > $imageData[0]) ? $imageData[0] : $size;
	}
	else
	{
		$size = ($size > $imageData[1]) ? $imageData[1] : $size;
	}
	$file = $img;
}
else
...
$hft_image = new hft_image($file);

if($sizeWidth)
	$hft_image->resize($size, '*', "-");
else
	$hft_image->resize('*', $size, "-");
	
$hft_image->output_resized("");
?>